...
import styles from './BrothersTrainer.module.css';
...
function generateSession(settings: BrothersSettings): BrothersSession {
  ...
  for (let i = 0; i < settings.count; i++) {
    ...
    if (i < settings.count / 2) {
      // Первая половина - больше положительных
      op = Math.random() < 0.6 ? '+' : '-';
    } else {
      // Вторая половина - больше отрицательных для баланса
      op = Math.random() < 0.4 ? '+' : '-';
    }
      ...
      <div className={styles.trainerContainer}>
        <div className={styles.headerSection}>
          ...
          <div className={styles.titleSection}>
            <h2 className={styles.trainerTitle}>Тренажер «Братья»</h2>
...
export default BrothersTrainer;
